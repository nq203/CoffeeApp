...
export const getListCoffeeShopByUserFavorite = async (
  userFavorites: number[]
): Promise<CoffeeShop[]> => {
  try {
    const allShops = await getAllCoffeeShops(); // Get all coffee shops
    if (!userFavorites.length) return [];

    const filtered = allShops.filter((shop: CoffeeShop) =>
      shop.utilities.some((util) => userFavorites.includes(util))
    );

    return filtered;
  } catch (error) {
    console.error("Error getting favorite coffee shops:", error);
    return [];
  }
};
// Cập nhật quán cafe
